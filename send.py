...
def send_s(udpLink, addr, i, address, iface, s, agg_idx):
    byteContent = pack_udp_packet_agg(addr, iface, s, agg_idx, i)
    udpLink.sendto(byteContent, addr)

def send_end(udpLink, addr, i,  address, iface, s, agg_idx):
    byteContent = pack_udp_packet_agg_end(addr, iface, s, agg_idx, 90000)
    udpLink.sendto(byteContent, addr)

def send_drop(udpLink, addr, i, address, iface, s, agg_idx):
    byteContent = pack_udp_packet_drop(addr, iface, s, agg_idx, i)
    udpLink.sendto(byteContent, addr)



def send_socket_forward(num, address, port, iface, s, agg_idx):
    udpLink = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    addr = (address, port)
    threads = []
    for i in range(0, num):
        thread = threading.Thread(target=send_s, args=(udpLink, addr, i, address, iface, s, agg_idx))
    ...
    send_end(udpLink, addr, num + 1, address, iface, s, agg_idx)

def send_socket_drop(num, address, port, iface, s, agg_idx):
    udpLink = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    addr = (address, port)
    threads = []
    for i in range(0, num):
        thread = threading.Thread(target=send_drop, args=(udpLink, addr, i, address, iface, s, agg_idx))
        thread.setDaemon(False)
        threads.append(thread)
        thread.start()
        time.sleep(0.0001)
...
def send_socket_congestion(num, address, port, iface, s, agg_idx):
    udpLink = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    addr = (address, port)
    ...
    for j in range(0, it):
        ...
        for i in range(0, max_it):
            thread = threading.Thread(target=send_s, args=(udpLink, addr, it*1000 + j, address, iface, s, agg_idx))
            thread.setDaemon(False)
            threads.append(thread)
            thread.start()
            time.sleep(0.01)
...
def pack_udp_packet_agg(addr, iface, s, agg_idx, index):
    bin_data = ""
    i = 0
    print(s)
    for p in s:
    ...
    bin_data += byteSeries(1, 8) + byteSeries(1, 8) + byteSeries(index, 32) + byteSeries(1, 32) + byteSeries(1, 48) + byteSeries(1, 48)
    for i in range(1, 341):
        bin_data += byteSeries(i, 32)
    byteContent = BitArray('0b' + bin_data).bytes
    return byteContent

def pack_udp_packet_agg_end(addr, iface, s, agg_idx, index):
    bin_data = ""
    i = 0
    print(s)
    for p in s:
    ...
    bin_data += byteSeries(0, 8) + byteSeries(1, 8) + byteSeries(index, 32) + byteSeries(1, 32) + byteSeries(1, 48) + byteSeries(1, 48)
...
def pack_udp_packet_drop(addr, iface, s, agg_idx, index):
    bin_data = ""
    i = 0
    print(s)
    for p in s:
...
def main():
    ...
    if len(sys.argv)<6:
        print 'pass 6 arguments: <ports>, <agg_idx>, <whether_forward> '
        exit(1)
    ...
    if whether_forward == 1:
        send_socket_forward(num, addr, dest_port, iface, s, agg_idx)
        # send_socket_congestion(num, addr, dest_port, iface, s, agg_idx)
    else:
        send_socket_drop(num, addr, dest_port,  iface, s, agg_idx)
    end = time.time()
    print end-begin
